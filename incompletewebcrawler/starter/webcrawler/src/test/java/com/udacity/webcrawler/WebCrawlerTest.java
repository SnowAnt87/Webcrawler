  ...
  public void zeroMaxDepth(Class<?> crawlerClass) {
    CrawlerConfiguration config =
        new CrawlerConfiguration.Builder()
            ...
            .addStartPages(List.of(Paths.get(DATA_DIR, "test-page.html").toUri().toString()))
            .build();
    Guice.createInjector(new WebCrawlerModule(config), new NoOpProfilerModule())
        .injectMembers(this);
  ...
  public void respectsIgnoredUrls(Class<?> crawlerClass) {
    CrawlerConfiguration config =
        new CrawlerConfiguration.Builder()
            ...
            .setMaxDepth(10)
            .setPopularWordCount(3)
            .addStartPages(List.of(
                Paths.get(DATA_DIR, "test-page.html").toUri().toString(),
                Paths.get(DATA_DIR, "infinite-loop.html").toUri().toString()))
            .addIgnoredUrls(List.of(".*-loop\\.html$", ".*dead-.*"))
            .build();
    Guice.createInjector(new WebCrawlerModule(config), new NoOpProfilerModule())
        .injectMembers(this);
    assertThat(crawler.getClass()).isAssignableTo(crawlerClass);
  ...
  public void pageNotFoundStillCountsAsVisited(Class<?> crawlerClass) {
    CrawlerConfiguration config =
        new CrawlerConfiguration.Builder()
            ...
            .setMaxDepth(10)
            .setPopularWordCount(20)
            .addStartPages(List.of(Paths.get(DATA_DIR, "link-1.html").toUri().toString()))
            .build();
    Guice.createInjector(new WebCrawlerModule(config), new NoOpProfilerModule())
        .injectMembers(this);
    assertThat(crawler.getClass()).isAssignableTo(crawlerClass);
  ...
  public void infiniteLoop(Class<?> crawlerClass) {
    CrawlerConfiguration config =
        new CrawlerConfiguration.Builder()
            ...
            .setPopularWordCount(3)
            .setTimeoutSeconds(1)
            .addStartPages(List.of(Paths.get(DATA_DIR, "infinite-loop.html").toUri().toString()))
            .build();
    Guice.createInjector(new WebCrawlerModule(config), new NoOpProfilerModule())
        .injectMembers(this);
    assertThat(crawler.getClass()).isAssignableTo(crawlerClass);
  ...
  public void multipleStartingUrls(Class<?> crawlerClass) {
    CrawlerConfiguration config =
        new CrawlerConfiguration.Builder()
            ...
            .addStartPages(List.of(
                Paths.get(DATA_DIR, "test-page.html").toUri().toString(),
                Paths.get(DATA_DIR, "link-1.html").toUri().toString(),
                Paths.get(DATA_DIR, "infinite-loop.html").toUri().toString()
            ))
